...
WORKDIR /go/src/github.com/pressrelations/docker-redis-logging-plugin
...
COPY . /go/src/github.com/pressrelations/docker-redis-logging-plugin
RUN go get
RUN go build --ldflags '-extldflags "-static"' -o /usr/bin/docker-redis-logging-plugin
